...
public class AStarDeathMap : MonoBehaviour
{

    public DeathMapController deathMapController;
    ...
    private void Awake()
    {
        ...
        deathMapController = GameObject.Find("DeathMapController").GetComponent<DeathMapController>();
        ...
        while (openList.Count != 0)
        {
            ...
            else
            {
                for (int i = 0; i < adjacentTiles.Count; i++)
                {
                    ...
                    if (LMFF.map[newNode.x, newNode.y] == LoadMapFromFile.sWall || LMFF.map[newNode.x, newNode.y] == LoadMapFromFile.sWallEnd || LMFF.map[newNode.x, newNode.y] == LoadMapFromFile.sWallRotated)
                    {
                    ...
                    if (compareCost(openList, newNode.x, newNode.y, NewCost) || compareCost(closedList, newNode.x, newNode.y, NewCost))
                    //if(searchList(openList, newNode.x, newNode.y) || searchList(closedList, newNode.x, newNode.y) && newNode.costFromStart <= NewCost)
                    {
                        continue;
                    }
    ...
    float TraverseCost(mapNode currentNode, mapNode newNode) // heuristic value
    {
        try
        {
            return deathMapController.threatMap[newNode.x, newNode.y] * 20; // this is multiplied as the value of the threat  map is very low, (0-1), the hiugher the multiplying factor, the more the agents will avoid danger
        }
