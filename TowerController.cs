...
public class TowerController : StationaryObject
{
    ...
    void Update()
    {
        ...
        if (currentDefenceState == defenceState.attacking)
        {
            ...
            else
            {
                ...
                if (attackTimer <= 0.0f)
                {
                    Vector3 cannonballPosition = new Vector3(transform.position.x, transform.position.y + 1.3f, transform.position.z);
